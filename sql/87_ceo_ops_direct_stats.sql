-- ============================================================================
-- TGT Nexus CRM — 87_ceo_ops_direct_stats.sql
-- CEO dashboard: separate pipeline stats for leads added directly by OPS QA
-- (lead_origin = 'ops_manual'). Keeps 85's data-source-mix logic. Safe to
-- re-run.
-- ============================================================================
...
begin
  ...
  select jsonb_build_object(
    ...
    'opsDirect', jsonb_build_object(
      'added', (
        select count(*) from leads
        where lead_origin = 'ops_manual' and private.in_tf(date_created, tf)
      ),
      'opsApproved', (
        select count(*) from ops_verifications o
        join leads l on l.lead_id = o.lead_id
        where l.lead_origin = 'ops_manual'
          and o.ops_status = 'Approved'
          and private.in_tf(o.ops_date, tf)
      ),
      'mspApproved', (
        select count(*) from msp_onboarding m
        join leads l on l.lead_id = m.lead_id
        where l.lead_origin = 'ops_manual'
          and m.final_status = 'Approved'
          and private.in_tf(m.ops_approved_date, tf)
      ),
      'funded', (
        select count(*) from leasing x
        join leads l on l.lead_id = x.lead_id
        where l.lead_origin = 'ops_manual'
          and x.funding_status = 'Funded'
          and private.in_tf(coalesce(x.funding_date, x.order_activation), tf)
      ),
      'live', (
        select count(*) from fulfillment f
        join leads l on l.lead_id = f.lead_id
        where l.lead_origin = 'ops_manual'
          and f.fulfillment_stage = 'Live'
          and private.in_tf(f.live_date, tf)
      )
    ),
    'qaQualified', (
