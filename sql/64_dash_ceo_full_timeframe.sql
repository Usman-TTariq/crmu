-- ============================================================================
-- TGT Nexus CRM — 64_dash_ceo_full_timeframe.sql
-- CEO dashboard: apply timeframe (dropdown / calendar day) to ALL metrics,
-- not only the top lead/QA boxes. Funnel, health rings, stages, drops, etc.
-- Safe to re-run.
-- ============================================================================
...
begin
  ...
  select jsonb_build_object(
    ...
    'revenue', (
      select coalesce(sum(approved_funding), 0) from leasing
      where funding_status = 'Funded'
        and private.in_tf(coalesce(funding_date, order_activation), tf)
    ),
    'approvedMids', (
