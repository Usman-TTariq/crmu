...
-- Sales-side write access (admin or sales head)
create or replace function private.sales_writer()
returns boolean language sql stable security definer set search_path = public as $$
  select private.is_admin() or private.role_key() = 'sales_head';
$$;
...
-- Read access to everything on the sales side (admin, sales head see all)
