-- ============================================================================
-- TGT Nexus CRM — 80_monitor_pacific_tz.sql
-- Employee Monitor day buckets: Asia/Karachi → America/Los_Angeles (Pacific).
-- Redefines presence heartbeat / breaks / dash RPCs. Safe to re-run.
-- Paste into Supabase SQL Editor after deploying app changes.
-- ============================================================================
...
begin
  ...
  if found and prev.last_heartbeat_at is not null then
    ...
    day_local := (now() at time zone 'America/Los_Angeles')::date;
...
as $$
declare
  day_local date := coalesce(p_day, (now() at time zone 'America/Los_Angeles')::date);
...
begin
  ...
  day_start := (day_local::timestamp at time zone 'America/Los_Angeles');
