-- ============================================================================
-- TGT Nexus CRM — 66_hr_role_monitor.sql
-- HR role: same full Employee Monitor board as CEO / super_admin.
-- Also accepts legacy role_key hr_monitor. Safe to re-run.
-- ============================================================================
...
as $$
  select private.is_admin()
      or private.role_key() in ('sales_head', 'ops_manager', 'hr', 'hr_monitor');
$$;
...
as $$
  select exists (
    ...
    where p.user_id = p_user_id
      ...
      and (
        private.is_admin()
        or private.role_key() in ('hr', 'hr_monitor')
        or (
...
-- Align existing HR monitor profiles
update public.profiles
set role_key = 'hr',
    title = 'HR',
    dept = 'ALL'
where role_key in ('hr_monitor', 'hr')
   or lower(full_name) in ('hafsa hussain', 'ahsan yousuf');
